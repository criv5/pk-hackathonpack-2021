...
public class EarthRift extends EarthAbility implements AddonAbility, ComboAbility {
	...
	public EarthRift(Player player) {
		...
		this.isEnabled = ConfigManager.getConfig().getBoolean("ExtraAbilities.Hiro3.HackathonPack.Earth.EarthRift.Enable");
		if (!isEnabled)
			return;
	...
	public void setField() {
		cooldown = ConfigManager.getConfig().getLong("ExtraAbilities.Hiro3.HackathonPack.Earth.EarthRift.Cooldown");
		amount = ConfigManager.getConfig().getInt("ExtraAbilities.Hiro3.HackathonPack.Earth.EarthRift.Amount");
		heightRepeat = ConfigManager.getConfig().getInt("ExtraAbilities.Hiro3.HackathonPack.Earth.EarthRift.HeightRepeat");
		if (heightRepeat < 1)
			heightRepeat = 1;
		currentHeight = 1;
		range = ConfigManager.getConfig().getDouble("ExtraAbilities.Hiro3.HackathonPack.Earth.EarthRift.SelectRange");
	...
	public void progress() {
		...
		if ( (targetBlock != null && !(targetBlock.getX() == headBlock.getX() && targetBlock.getZ() == headBlock.getZ())) || targetBlock == null ) {
			if (targetBlock != null) 
				...
				blockDirection = getTargetLocation(player, 25).subtract(headBlock.getLocation()).toVector().normalize();
			}
			...
			tmpI = 0;
			safeLoc = tmpLoc.clone();
			while (!GeneralMethods.isSolid(tmpLoc.getBlock()) && tmpI < 10) {
				tmpLoc.setY(tmpLoc.getY() - 1);
				tmpI++;
			}
			if (tmpI >= 10) {
				tmpLoc = safeLoc.clone();
			}
	...
	public void load() {
		...
		ConfigManager.getConfig().addDefault("ExtraAbilities.Hiro3.HackathonPack.Earth.EarthRift.Enable", true);
		ConfigManager.getConfig().addDefault("ExtraAbilities.Hiro3.HackathonPack.Earth.EarthRift.Cooldown", 5000);
		ConfigManager.getConfig().addDefault("ExtraAbilities.Hiro3.HackathonPack.Earth.EarthRift.Amount", 15);
		ConfigManager.getConfig().addDefault("ExtraAbilities.Hiro3.HackathonPack.Earth.EarthRift.HeightRepeat", 2);
		ConfigManager.getConfig().addDefault("ExtraAbilities.Hiro3.HackathonPack.Earth.EarthRift.SelectRange", 50);
