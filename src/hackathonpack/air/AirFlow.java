...
import hackathonpack.MainListener;
import hackathonpack.UtilityMethods;

public class AirFlow extends AirAbility implements AddonAbility, ComboAbility {
	...
	public AirFlow(Player player) {
		...
		this.isEnabled = ConfigManager.getConfig().getBoolean("ExtraAbilities.Hiro3.HackathonPack.Air.AirFlow.Enable");
		if (!isEnabled)
			return;
	...
	public void setField() {
		...
		this.chargeTime = ConfigManager.getConfig().getLong("ExtraAbilities.Hiro3.HackathonPack.Air.AirFlow.ChargeTime");
		this.duration = ConfigManager.getConfig().getLong("ExtraAbilities.Hiro3.HackathonPack.Air.AirFlow.Duration");
		this.cooldown = ConfigManager.getConfig().getLong("ExtraAbilities.Hiro3.HackathonPack.Air.AirFlow.Cooldown");
		this.range = ConfigManager.getConfig().getDouble("ExtraAbilities.Hiro3.HackathonPack.Air.AirFlow.Range");
		this.particlePerTick = ConfigManager.getConfig().getInt("ExtraAbilities.Hiro3.HackathonPack.Air.AirFlow.ParticlePerTick");
		this.particleSpawnAreaSize = ConfigManager.getConfig().getDouble("ExtraAbilities.Hiro3.HackathonPack.Air.AirFlow.ParticleSpawnAreaSize");
	...
	public void progress() {
		...
		if (abilityState == AbilityState.CHARGING) {
			if (!player.isSneaking()) {
				...
				if (Math.random() < 0.2)
				player.spawnParticle(Particle.SPELL, player.getEyeLocation().add(player.getLocation().getDirection().multiply(4)), 0);
			}
		} else if (abilityState == AbilityState.CHARGED) {
			if (!player.isSneaking()) {
				abilityState = AbilityState.LAUNCHED;
				this.launchTime = System.currentTimeMillis();
				this.flowLocation = player.getEyeLocation().add(player.getLocation().getDirection().multiply(4));
				this.flowDirection = player.getLocation().getDirection();
				player.getWorld().playSound(this.flowLocation, Sound.ITEM_ELYTRA_FLYING, 0.05f, 1);
				bPlayer.addCooldown(this);
			} else {
				//Charged particle codes
				player.spawnParticle(Particle.SPELL, player.getEyeLocation().add(player.getLocation().getDirection().multiply(4)), 0, 0.5f, 0.5f, 0.5f);
			}
	...
	public String getDescription() {
		return "Create a flow of air that can carry entities.";
	}
	...
	public void load() {
		...
		ConfigManager.getConfig().addDefault("ExtraAbilities.Hiro3.HackathonPack.Air.AirFlow.Enable", true);
		ConfigManager.getConfig().addDefault("ExtraAbilities.Hiro3.HackathonPack.Air.AirFlow.Cooldown", 5000);
		ConfigManager.getConfig().addDefault("ExtraAbilities.Hiro3.HackathonPack.Air.AirFlow.Duration", 10000);
		ConfigManager.getConfig().addDefault("ExtraAbilities.Hiro3.HackathonPack.Air.AirFlow.ChargeTime", 500);
		ConfigManager.getConfig().addDefault("ExtraAbilities.Hiro3.HackathonPack.Air.AirFlow.Range", 20);
		ConfigManager.getConfig().addDefault("ExtraAbilities.Hiro3.HackathonPack.Air.AirFlow.ParticlePerTick", 1);
		ConfigManager.getConfig().addDefault("ExtraAbilities.Hiro3.HackathonPack.Air.AirFlow.ParticleSpawnAreaSize", 3);
